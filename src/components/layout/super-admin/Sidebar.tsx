...
export function AppSidebar() {
    ...
    return (
        <Sidebar
            variant="sidebar"
            collapsible="none"
            className="bg-[var(--sidebar)] text-[var(--sidebar-foreground)] border-r border-[var(--sidebar-border)]"
        >
                        ...
                        <SidebarMenu className="gap-1">
                            {items.map((item) => (
                                <SidebarMenuItem key={item.title}>
                                    <SidebarMenuButton
                                        asChild
                                        isActive={location.pathname === item.url || (item.url !== '/super-admin' && location.pathname.startsWith(item.url))}
                                        tooltip={item.title}
